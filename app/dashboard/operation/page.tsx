                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {reservations.map((res) => (
                            <div key={res.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl p-6 hover:border-amber-300 dark:hover:border-amber-500/40 transition-all group relative overflow-hidden shadow-sm dark:shadow-2xl">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-amber-400/10 to-blue-500/10 dark:from-purple-500/10 dark:to-blue-500/10 rounded-bl-full -mr-10 -mt-10 transition-all group-hover:scale-110"></div>

                                {/* Header */}
                                <div className="flex items-start justify-between mb-6 relative">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-50 to-slate-100 dark:from-slate-800 dark:to-slate-950 flex flex-col items-center justify-center text-lg font-black text-slate-900 dark:text-white shadow-inner border border-slate-200 dark:border-white/10">
                                            {formatDate(res.date).day}
                                            <span className="text-[10px] block -mt-1 text-slate-500 dark:text-slate-400 uppercase">{formatDate(res.date).month}</span>
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs font-mono text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded-lg">{res.voucher_number}</span>
                                            ...
                                            <h3 className="font-bold text-slate-950 dark:text-white text-lg mt-1">{res.customer_name}</h3>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">{res.customer_phone}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 relative">
                                    <div className="bg-slate-50 dark:bg-slate-950/60 rounded-2xl p-4 border border-slate-200 dark:border-white/10 space-y-3">
                                        <div className="flex items-start gap-3">
                                            <span className="text-app-subtle mt-1">📍</span>
                                            <div>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-black tracking-wider">KARŞILAMA</p>
                                                <p className="text-sm font-bold text-slate-900 dark:text-slate-50">{res.pickup_location}</p>
                                        ...
                                        <div className="flex items-start gap-3">
                                            <span className="text-app-subtle mt-1">🏁</span>
                                            <div>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-black tracking-wider">VARIŞ</p>
                                                <p className="text-sm font-bold text-slate-900 dark:text-slate-50">{res.dropoff_location}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between px-2">
                                        ...
                                        <div className="text-right">
                                            <p className="text-xs text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">FİYAT</p>
                                            <p className="text-lg font-black text-slate-950 dark:text-white">{res.price} <span className="text-sm text-slate-500 dark:text-slate-400">{res.currency}</span></p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="flex-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-white/10 dark:hover:bg-white/15 dark:text-slate-200 text-[11px] font-black rounded-xl border border-slate-200 dark:border-white/10 transition-all flex items-center justify-center gap-2"
                                            >
                                        ...
                                        <div className="flex justify-between items-center gap-2">
                                            <div className="flex gap-1">
                                                <button onClick={() => { setEditRes(res); setShowAddModal(true); }} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-white/10 dark:hover:bg-white/15 dark:text-slate-300 flex items-center justify-center transition-colors">✏️</button>
                                                <button onClick={() => handleDeleteReservation(res.id)} className="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-900/20 dark:hover:bg-red-900/40 dark:text-red-400 flex items-center justify-center transition-colors">🗑️</button>
                                            </div>
                                        </div>
                                    </div>
                                    {res.driver_phone && (
